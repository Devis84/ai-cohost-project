...
with villa_seed as (
  ...
  from (
    values
      (1, 'Tropikal Villa 1', 'tropikal-villa-1'),
      (2, 'Tropikal Villa 2', 'tropikal-villa-2'),
      (3, 'Tropikal Villa 3', 'tropikal-villa-3'),
      (4, 'Tropikal Villa 4', 'tropikal-villa-4')
  ) as v(villa_number, villa_name, villa_slug)
)
...
select
  ...
  jsonb_build_object(
    'guest_page', jsonb_build_object(
      'hero_title', 'Welcome to ' || villa_name,
      ...
      'about_intro', villa_name || ' is part of the Indonesia partner test property group.',
...
where not exists (
  ...
  where p.slug = villa_seed.villa_slug
);
...
values (
  ...
  jsonb_build_object(
    ...
    'status', 'foundation_created_without_partner_email',
    ...
    'next_step', 'Create admin and partner user profiles, then assign partner access to Tropikal Villa 1-4.'
  )
);
