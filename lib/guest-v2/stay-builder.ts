...
function formatCheckoutStatus(checkoutDate: unknown) {
  if (typeof checkoutDate !== "string" || !checkoutDate) {
    return "Checkout in 3 days";
  }

  const today = new Date();
  const checkout = new Date(checkoutDate);

  if (Number.isNaN(checkout.getTime())) return "Checkout in 3 days";

  const diffMs = checkout.getTime() - today.getTime();
  const diffDays = Math.ceil(diffMs / (1000 * 60 * 60 * 24));

  if (diffDays < 0) return "Stay ended";
  if (diffDays === 0) return "Checkout today";
  if (diffDays === 1) return "Checkout tomorrow";

  return `Checkout in ${diffDays} days`;
}
    ...
    if (section.slug === "emergency") {
      ...
      return {
        ...
        items: [
          {
            title: "Host contact",
            body: hostContact || "Host contact details will appear here once added.",
            icon: "🟢",
          },
          {
...
export async function buildGuestV2StayFromToken(
  ...
  const stayStatusLabel = formatCheckoutStatus(accessRecord.checkout_date);
  ...
  return {
    ...
    today: {
      items: [
        {
          icon: access.allowed ? "✅" : "🔒",
          label: access.allowed ? "Guest access active" : access.title,
        },
        {
          icon: "🌤️",
