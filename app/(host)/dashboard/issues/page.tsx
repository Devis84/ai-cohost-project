...
function getStatusClass(status: string) {
  ...
  if (normalized === "resolved" || normalized === "closed") return "bg-emerald-50 text-emerald-700 border-emerald-100"
...
function getIssueTone(issue: Issue) {
  ...
  if (
    ...
    return { icon: "🚨", label: "Emergency / Safety" }
  }
  ...
  if (
    ...
    return { icon: "🔑", label: "Access / Check-in" }
  }

  if (combined.includes("wifi") || combined.includes("internet")) {
    return { icon: "📶", label: "WiFi / Internet" }
  }
  ...
  if (
    ...
    return { icon: "🛠️", label: "Maintenance" }
  }
  ...
  if (
    ...
    return { icon: "⚠️", label: "Complaint" }
  }

  return { icon: "💬", label: "Guest Support" }
}
...
function FilterButton({
  ...
  return (
    ...
    >
      {label}
      <span className={`ml-2 ${active ? "text-white/60" : "text-gray-400"}`}>
        {count}
      </span>
    </button>
  )
  ...
  const filteredIssues = useMemo(() => {
    ...
    return issues.filter((issue) => {
      const matchesFilter =
        ...
        (filter === "high" && !isResolved(issue) && isHighPriority(issue)) ||
        (filter === "medium" && !isResolved(issue) && isMediumPriority(issue))
      ...
      <div className="sticky top-0 z-40 backdrop-blur-2xl bg-white/80 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col md:flex-row md:items-center md:justify-between gap-5">
          <div>
            ...
            <h1 className="text-3xl font-bold text-gray-900">Guest Problem Center</h1>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <div className="bg-gradient-to-br from-black via-zinc-900 to-zinc-800 text-white rounded-[32px] p-7 md:p-8 shadow-2xl mb-8">
          <div className="grid md:grid-cols-4 gap-4">
            <div className="bg-white/10 border border-white/10 rounded-3xl p-5">
              <div className="text-white/50 text-sm mb-2">Open Issues</div>
              <div className="text-4xl font-black">{openIssues.length}</div>
            </div>

            <div className="bg-white/10 border border-white/10 rounded-3xl p-5">
              <div className="text-white/50 text-sm mb-2">High Priority</div>
              <div className="text-4xl font-black">{highPriorityIssues.length}</div>
            </div>

            <div className="bg-white/10 border border-white/10 rounded-3xl p-5">
              <div className="text-white/50 text-sm mb-2">Medium</div>
              <div className="text-4xl font-black">{mediumPriorityIssues.length}</div>
            </div>

            <div className="bg-white/10 border border-white/10 rounded-3xl p-5">
              <div className="text-white/50 text-sm mb-2">Resolved</div>
              <div className="text-4xl font-black">{resolvedIssues.length}</div>
            </div>
          </div>
        </div>
        ...
        {!loading && !errorMessage && filteredIssues.length === 0 && (
          <div className="bg-white rounded-3xl border border-gray-200 shadow-xl p-10 text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-3">No issues in this view</h2>
        ...
        {!loading && !errorMessage && filteredIssues.length > 0 && (
          <div className="grid xl:grid-cols-2 gap-5">
            {filteredIssues.map((issue) => {
              ...
              const issueTitle =
                issue.issue_type || issue.severity || issue.priority || "Guest issue"

              const description =
                issue.description || issue.message || "No description provided."

              const propertyName =
                issue.property_name ||
                (issue.property_id ? `Property ${issue.property_id.slice(0, 8)}` : "Unknown property")
                ...
                >
                  <div className="flex flex-col gap-5">
                    <div className="flex items-start justify-between gap-5">
                      <div>
                        <div className="text-4xl mb-3">{issueTone.icon}</div>
                    ...
                    <div className="bg-gray-50 border border-gray-100 rounded-3xl p-5">
                      ...
                      <div className="font-bold text-gray-900">{propertyName}</div>
                      {propertyCity && (
                        <div className="text-sm text-gray-500 mt-1">{propertyCity}</div>
                      )}
                      {issue.guest_name && (
