...
export async function POST(
  ...
  return handleIdempotentTransition(requestId, 'resolved', user.id, async () => {
    let updated
    try {
      updated = await updateGuestRequestStatus(requestId, 'resolved', user.id)
    } catch (err: unknown) {
    ...
    // and the response still reflects the committed resolved state.
    try {
      await cancelEscalation(requestId)
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : 'cancelEscalation failed'
      console.error('[resolve] cancelEscalation failed after status update', {
        requestId,
        error: message,
      })
    }
